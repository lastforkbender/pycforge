...
class Phase14CKeywordLoweringTests(unittest.TestCase):
    ...
    def test_values_stage_in_source_order_and_call_references_use_formal_order(self) -> None:
        source = (
    ...
    def test_nested_calls_execute_in_keyword_source_order_before_formal_call(self) -> None:
        source = (
    ...
    def test_keyword_call_preludes_remain_inside_phase14b_boolean_guard(self) -> None:
        source = (
            "def flag(value: bool) -> bool:\n    return value\n\n"
            "def pair(left: bool, right: bool) -> bool:\n    return left\n\n"
            "def run(a: bool, b: bool) -> bool:\n"
            "    return a and pair(right=flag(b), left=a)\n"
        )
        payload = self.converted(source).stage_artifact.payload
        names = binding_names(payload)
        run = function(payload, "run")
        ...
        self.assertEqual(inside, ["flag", "pair"])
    ...
    def test_keyword_call_preludes_remain_inside_phase14b_chain_guard(self) -> None:
        source = (
            "def choose(left: int, right: int) -> int:\n    return left\n\n"
            "def run(a: int, b: int, c: int) -> bool:\n"
            "    return a < b < choose(right=c, left=b)\n"
        )
        payload = self.converted(source).stage_artifact.payload
        names = binding_names(payload)
        run = function(payload, "run")
        ...
        self.assertEqual(inside, ["choose"])
        self.assertEqual(inside, all_calls)

    def test_numeric_helper_keyword_value_keeps_source_staging_and_helper_closure(self) -> None:
        source = (
    ...
    def test_container_and_record_reads_are_valid_keyword_values(self) -> None:
        sources = {
            "container": (
                "def sink(value: int, flag: bool) -> int:\n    return value\n\n"
                "def run(flag: bool) -> int:\n"
                "    values = [10, 20]\n"
                "    return sink(flag=flag, value=values[1])\n"
            ),
            "record": (
                "class Sample:\n"
                "    count: int\n"
                ...
                "def run(value: int, flag: bool) -> int:\n"
                "    sample = Sample(value)\n"
                "    return sink(flag=flag, value=sample.count)\n"
            ),
        }
        expected_expression_kind = {
            "container": "CSubscriptExpr",
            "record": "CMemberAccessExpr",
        }
        for label, source in sources.items():
            with self.subTest(label=label):
                ...
                names = binding_names(payload)
                run = function(payload, "run")
                sink = call_named(run, names, "sink")
                argument_ids = {item["binding_id"] for item in sink["arguments"]}
                staging = [
                    item
                    for item in walk(run["body"])
                    if item.get("kind") == "CVariableDeclaration"
                    and item.get("identifier", {}).get("binding_id") in argument_ids
                ]
                self.assertEqual(len(staging), 2)
                self.assertTrue(
                    any(
                        item.get("kind") == expected_expression_kind[label]
                        for item in walk(staging)
                    )
                )
                feature = table(payload, "keyword-call-binding-facts")["records"]
                self.assertEqual(len(feature), 1)
                self.assertTrue(feature[0]["value"]["supported"])
