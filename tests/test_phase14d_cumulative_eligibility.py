...
class Phase14DCumulativeEligibilityTests(unittest.TestCase):
        ...
        *,
        ...
        for fact in facts:
            with self.subTest(call_node_id=fact["call_node_id"]):
                ...
                self.assertEqual(fact["reason"], CUMULATIVE_KEYWORD_ONLY_TARGET_REASON)
                self.assertEqual(
                ...
                self.assertIsNone(decisions[fact["call_node_id"]]["rule_plan_id"])
        self.assertEqual(
            [
                item
    ...
    def test_target_ineligibility_propagates_through_supported_direct_calls(self) -> None:
        self.assert_cumulative_rejection(
            ...
            "        return middle(value)\n"
            "    return middle(value)\n\n"
            "def entry(value: int, flag: bool) -> int:\n"
            "    return run(flag=flag, value=value)\n",
            "PYC2930",
        )

    def test_long_invalid_target_chain_closes_without_rescan_waves(self) -> None:
        function_count = 180
        definitions = [
        ...
        definitions.append(
            "def target(value: int, *, flag: bool) -> int:\n"
            "    if flag:\n"
            "        return step_0(value)\n"
            "    return step_0(value)\n"
        )
        definitions.append(
            "def run(value: int, flag: bool) -> int:\n"
            "    return target(flag=flag, value=value)\n"
        )
        self.assert_cumulative_rejection("\n".join(definitions), "PYC2930")

    def test_declaration_default_rejection_is_not_rewritten_as_cumulative(self) -> None:
        result = convert(
    ...
    def test_validation_cancellation_names_the_exact_component(self) -> None:
