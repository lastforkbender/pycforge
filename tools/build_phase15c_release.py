"""Build the deterministic, source-only PyCForge Phase 15C release.
...
SOURCE_ARCHIVE_NAME = "pycforge_phase_15c_v0_15_2.tar.gz"
SOURCE_ARCHIVE_ROOT = "pycforge_phase_15c_v0_15_2"
WHEEL_NAME = "pycforge-0.15.2-py3-none-any.whl"
HANDOFF_NAME = "PyCForge_Phase_15C_v0_15_2_Project_Handoff.txt"
PACKAGE_REPORT_NAME = "PyCForge_Phase_15C_v0_15_2_Package_Report.json"
CHECKSUMS_JSON_NAME = "PyCForge_Phase_15C_v0_15_2_Checksums.json"
CHECKSUMS_TEXT_NAME = "PyCForge_Phase_15C_v0_15_2_Checksums.txt"
VALIDATION_REPORT_NAME = "PyCForge_Phase_15C_v0_15_2_Validation_Report.json"
INTERNAL_VALIDATION_REPORT = "evidence/phase_15c/validation_report.json"
RELEASE_FINGERPRINT = PurePosixPath(
    "transition/phase_15c/release_fingerprint.json"
)
TRANSITION_MANIFEST = PurePosixPath("transition/phase_15c/manifest.json")
FINGERPRINT_DOMAIN = "pycforge-phase15c-release-tree-v1"
VALIDATION_SUBJECT_DOMAIN = "pycforge-phase15c-validation-subject-v1"
...
EXPECTED_PROJECT_CONFIGURATION = {
    ...
    "project": {
        ...
        "dependencies": ["PyQt5>=5.15.11,<6"],
        "keywords": ["python", "c", "transpiler", "pyqt5", "desktop"],
        "classifiers": [
        ...
        "gui-scripts": EXPECTED_GUI_SCRIPTS,
        "urls": {
            ...
            "Changelog": (
                "https://github.com/lastforkbender/pycforge/"
...
REQUIRED_WHEEL_MEMBERS = frozenset(
    {
        ...
        "pycforge/ide/qt_command_palette.py",
        ...
        "pycforge/ide/qt_workspace_widgets.py",
        "pycforge/ide/revisions.py",
        "pycforge/ide/search_service.py",
        "pycforge/ide/session_history.py",
        "pycforge/ide/source_editing.py",
        ...
        "pycforge/ide/theme_workspace_stylesheet.py",
        "pycforge/ide/visual_tokens.py",
        "pycforge/ide/worker_protocol.py",
        "pycforge/ide/workspace_search.py",
        "pycforge/ide/workspace_session.py",
    }
)
...
def inspect_wheel(
    ...
    *,
    ...
    if len(svg) != 55:
        raise ReleaseBuildError(f"wheel contains {len(svg)} SVG assets, expected 55")
    ...
    license_name = unique("licenses/LICENSE")
    ...
    if metadata.get("Description-Content-Type") != "text/markdown":
        raise ReleaseBuildError("wheel long-description content type is not exact")
    if metadata.get("License-Expression") != "GPL-3.0-only":
        raise ReleaseBuildError("wheel license expression is not exact")
    if members[license_name] != (ROOT / "LICENSE").read_bytes():
        raise ReleaseBuildError("wheel GPLv3 license text is not exact")
    ...
    if parser.sections() != ["console_scripts", "gui_scripts"] or parser.defaults():
    ...
    gui_scripts = dict(parser.items("gui_scripts"))
    if gui_scripts != EXPECTED_GUI_SCRIPTS:
        raise ReleaseBuildError("wheel GUI scripts do not match")
    ...
    return {
        ...
        "gui_scripts": dict(sorted(gui_scripts.items())),
...
def main():
    ...
    assert WORKSPACE_CONTRACT_VERSION == "pycforge-workspace/0.5"
    assert ACTION_REGISTRY_VERSION == "pycforge.action-registry/0.2"
    assert VISUAL_SYSTEM_VERSION == "pycforge.visual-system/0.2"
...
def _package_smokes(
    ...
    source_script = source_root / "_phase15c_package_smoke.py"
...
def _validate_promotion_report(
    ...
    *,
    ...
    if any(
        ...
        raise ReleaseBuildError(
            "internal Phase 15C promotion validation is not exact"
        )
    ...
    if (
        ...
        raise ReleaseBuildError(
            "internal Phase 15C validation audits are not all passing"
        )
    audit_names = [
    ...
    if (
        ...
        raise ReleaseBuildError(
            "internal Phase 15C validation audit inventory is not exact"
        )
    by_name = {
    ...
    validation_subject = by_name["phase15c-validation-subject"]
    predecessor = by_name["phase15b-predecessor-authentication"]
    ...
    workspace = by_name["phase15c-workspace-completeness"]
    ...
    if (
        ...
        raise ReleaseBuildError("Phase 15B predecessor is not authenticated")
    if (
    ...
    if (
        actions.get("actions") != 48
        or actions.get("context_surfaces") != 11
        or actions.get("main_menu_surfaces") != 5
    ...
    if (
        workspace.get("source_document_limit") != 64
        or workspace.get("source_pane_limit") != 2
        or workspace.get("search_match_limit") != 5_000
        or workspace.get("structure_symbol_limit") != 4_096
        or workspace.get("command_palette_limit") != 50
        or workspace.get("history_entry_limit") != 64
        or workspace.get("latest_wins_observers") is not True
        or workspace.get("pending_sync_invalidates_observers") is not True
        or workspace.get("generated_c_read_only") is not True
        or workspace.get("generated_c_explicit_save_only") is not True
    ):
        raise ReleaseBuildError("workspace-completeness evidence is not exact")
    ...
    if (
        platform_scope.get("evidence_path")
        != "evidence/phase_15c/performance_evidence.json"
        or platform_scope.get("evidence_authenticated") is not True
        or type(platform_scope.get("evidence_file_bytes")) is not int
        or platform_scope.get("evidence_file_bytes", 0) <= 0
        or not isinstance(platform_scope.get("evidence_sha256"), str)
        or len(platform_scope["evidence_sha256"]) != 64
        or any(
            character not in "0123456789abcdef"
            for character in platform_scope["evidence_sha256"]
        )
        or platform_scope.get("real_qapplication_exercised") is not True
        or platform_scope.get("real_pyqt_widgets_exercised") is not True
        or platform_scope.get("offscreen_pyqt_widgets_exercised") is not True
        or platform_scope.get("gui_event_loop_timing_recorded") is not True
        or platform_scope.get("visible_windows_11_exercised") is not False
        or platform_scope.get("visible_linux_desktop_exercised") is not False
        or platform_scope.get("display_scaling_matrix_exercised") is not False
        or platform_scope.get("assistive_technology_exercised") is not False
...
def _validate_transition_manifest(manifest: object) -> None:
    if not isinstance(manifest, dict):
        raise ReleaseBuildError("Phase 15C transition manifest is not an object")
    expected_keys = {
        "schema",
        "phase",
        "version",
    ...
    if set(manifest) != expected_keys:
        raise ReleaseBuildError(
            "Phase 15C transition manifest field inventory is not exact"
        )
    expected = {
        "schema": "pycforge.phase15c-manifest/0.15.2",
        "phase": "15C",
        "version": RELEASE_VERSION,
    ...
    if any(
        ...
        raise ReleaseBuildError(
            "Phase 15C transition manifest is not promoted and exact"
        )
    expected_predecessor = {
        "phase": "15B",
        "version": "0.15.1",
        "archive": PREDECESSOR_ARCHIVE_NAME,
        "archive_root": PREDECESSOR_ARCHIVE_ROOT,
    ...
    expected_scope = {
        "document_tabs_identifier_only": True,
        "maximum_editor_panes": 2,
        "bounded_undoable_source_operations": True,
        "go_to_line_without_complete_source_copy": True,
        "whitespace_and_folding_projection": True,
        "outline_and_breadcrumbs_from_open_source_only": True,
        "bundle_literal_search_open_documents_only": True,
        "bundle_search_projection_cap": 5_000,
        "command_palette_declared_actions_only": True,
        "command_palette_projection_cap": 50,
        "payload_free_session_history": True,
        "session_history_cap": 64,
        "generated_c_read_only": True,
        "generated_c_explicit_save_only": True,
        "action_specifications": 48,
        "static_actions": 47,
        "bounded_dynamic_action_templates": 1,
        "declared_surfaces": 18,
        "required_context_surfaces": 11,
        "persistent_main_menus": 5,
        "packaged_scalable_svg_icons": 55,
        "phase_15a_process_isolation_preserved": True,
        "phase_15b_visual_and_action_foundation_preserved": True,
    }
    expected_required_files = [
        ...
        "pycforge/ide/workspace_search.py",
        "pycforge/ide/workspace_session.py",
        "transition/phase_15c/entry_criteria.md",
        "transition/phase_15c/workspace_completeness_decision.md",
        "transition/phase_15c/rollback_conditions.md",
        "transition/phase_15c/gate_evidence.md",
        "transition/phase_15c/manifest.json",
        RELEASE_FINGERPRINT.as_posix(),
        "evidence/phase_15c/feature_evidence.json",
        "evidence/phase_15c/action_registry_evidence.json",
        "evidence/phase_15c/performance_evidence.json",
        INTERNAL_VALIDATION_REPORT,
        "tools/_phase15c_release_contract.py",
        "tools/validate_phase15c.py",
        "tools/build_phase15c_release.py",
        "tests/test_phase12_2_qt.py",
        "tests/test_phase15c_action_contract.py",
        "tests/test_phase15c_command_palette.py",
        "tests/test_phase15c_qt_command_contract.py",
        "tests/test_phase15c_qt_panels_contract.py",
        "tests/test_phase15c_qt_position_reverse.py",
        "tests/test_phase15c_qt_workspace_integration.py",
        "tests/test_phase15c_session_history.py",
        "tests/test_phase15c_source_editing.py",
        "tests/test_phase15c_source_structure.py",
        "tests/test_phase15c_visual_system.py",
        "tests/test_phase15c_workspace_search.py",
        "tests/test_phase15c_workspace_session.py",
        "tests/test_phase15c_release_contract.py",
        "tests/test_validate_phase15c.py",
        "tests/test_phase15c_release_packaging.py",
    ]
    expected_artifacts = {
    ...
    expected_platform = {
        ...
        "real_pyqt_widgets": True,
        "offscreen_pyqt_widgets": True,
    ...
    expected_non_goals = [
        ...
        "generated-C editing",
        "language server, formatter, semantic completion, refactoring, or quick-fix mutation",
        "source-control hosting, remote files, collaboration, or cloud synchronization",
        "Phase 15D distribution and visible platform claims",
    ]
    expected_debt = {
        "id": "CE-IDE-WORKSPACE-COMPLETENESS",
        "status": "retired-for-phase-15c-scope",
        "real_platform_debt_retired": False,
        "distribution_debt_retired": False,
    }
    expected_validation = {
        "status": "promotion-passed",
        "canonical_report_assigned": True,
    ...
    if (
        ...
        or manifest.get("workspace_completeness_debt") != expected_debt
        or manifest.get("required_files") != expected_required_files
        or manifest.get("planned_release_artifacts") != expected_artifacts
        or manifest.get("platform_evidence") != expected_platform
        or manifest.get("validation") != expected_validation
        or manifest.get("non_goals") != expected_non_goals
    ):
        raise ReleaseBuildError(
            "Phase 15C transition manifest evidence is not exact"
        )
    promotion = manifest.get("promotion")
    required_true = (
        "phase_15c_implemented",
        "phase_15c_validated",
        "phase_15c_promoted",
        "phase_15c_sealed",
    ...
    if (
        ...
        raise ReleaseBuildError(
            "Phase 15C transition promotion flags are not exact"
        )
...
def _validate_project_configuration(
    ...
    return {
        ...
        "gui_scripts": dict(EXPECTED_GUI_SCRIPTS),
        "runtime_dependencies": ["PyQt5>=5.15.11,<6"],
...
def _validate_tree_records(
    ...
    manifest = strict_json_loads(
        ...
        label="Phase 15C transition manifest",
        canonical=False,
    )
...
def build_release(root: Path, output_directory: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="pycforge-phase15c-build-") as name:
        work = Path(name)
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pycforge-phase15c-stage-",
            dir=output.parent,
        ) as stage_name:
            ...
            report = {
                "schema": "pycforge.phase15c-release-report/0.15.2",
                "phase": "15C",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build deterministic PyCForge Phase 15C source-only artifacts."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema": "pycforge.phase15c-package-build-error/1",
                    "passed": False,
                    "error": str(exc),
                    "c_toolchain_invoked": False,
                    "generated_c_compiled_or_executed": False,
